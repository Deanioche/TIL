...
// operator with let -
    ...
    nullableName4?.let { println(it.toLowerCase()) } // .let{}는 ?.을 함께 사용할 경우 if (type != null) {} 을 대체할 수 있다.
    nullableName4?.let { println(it.length) } // null이 아닐때만 execute. null이면 무시
...
// Then we can use the elvis operator ?:
    val name2 = nullableName4 ?: "Guest" // null이면 Default Value 입력
...
// the variable.
    val nullableName5: String? = null // !!앞의 변수는 반드시 null이 아니어야 함
