...
- 2-3-4 트리는 한 노드 덩어리에서 진출하는 차수가 2~4개.

    ![image](https://user-images.githubusercontent.com/66513003/145520690-06bdca50-e568-4f10-803c-f25296bcd64f.png)

- 승패트리 차이점
    - 승자는 루트가 최하값, 패자는 최상값
    - 패자는 루트 노드 위에 최상위 0번 노드를 가짐
    - 승자는 그런거 없음.

- B트리
    - 루트가 가장 작은 값을 가지지는 않음
    - 부모는 자식보다 작은 값을 가지지는 않음

- m원 탐색 트리
    - v의 왼쪽 서브트리에 있는 모든 노드의 키 값은 v의 키값보다 작지 않음.

- 2-3트리의 노드 구조와 탐색 연산
    ![image](https://user-images.githubusercontent.com/66513003/145535517-509d81e8-57d2-4aee-bb86-7e1ed765e811.png)

- 트리 -> 2진 트리
    - 2진트리의 왼쪽은 찐자식, 오른쪽은 sibling이 옴
    - B는 A의 찐자식
    - 찐자식 자리는 B가 차지했으므로, 다음 깊이에서 C는 B의 sibling이므로 오른쪽.
    - 같은이유로 D도 오른쪽